...
export class OpenSheetMusicDisplay {
    ...
    public load(
      ...
      options: {
        ...
        if (typeof content === "string") {
            ...
            if (trimmedStr.slice(0, 6).includes("<?xml")) { // first character is sometimes null, making first five characters '<?xm'.
        ...
        if (!content ||  !(<Document>content).nodeName) {
    ...
    public setContainer(container: string | HTMLElement): void {
      ...
      if (container) {
        ...
        } else if (container && "appendChild" in <HTMLElement>container) {
      ...
      /*
        TODO: This was my custom workaround to allow OSMD without any container.
        Presumably, it is worked around in alternative way in the upstrea,
        thus this code can be removed. But it should be tested.
    ...
    public setOptions(options: IOSMDOptions): void {
        ...
        if (options.defaultFontFamily) {
          this.rules.DefaultFont = new Font(
            options.defaultFontFamily,
            undefined,
            options.defaultFontWeight || "normal",
            options.defaultFontItalic || false,
            false,
          );
        }
