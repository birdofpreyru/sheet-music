...
export class SkyBottomLineCalculator {
    ...
    public updateLines(calculationResults: SkyBottomLineCalculationResult[]): void {
        ...
        this.mSkyLine = this.mSkyLine.map(v => (v - lowestSkyLine) / EngravingRules.UnitToPx + this.StaffLineParent.TopLineOffset);

        const highestBottomLine: number = Math.min(...this.mBottomLine);
        this.mBottomLine = this.mBottomLine.map(v => (v - highestBottomLine) / EngravingRules.UnitToPx + this.StaffLineParent.BottomLineOffset);
    }
